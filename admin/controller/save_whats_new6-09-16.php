...
define("VIDEOS", 2);
define("REVIEWS", 3);
define("BASE_DIR", $_SERVER['DOCUMENT_ROOT'] . "/suncros-new/admin/");
...
if (isset($_SESSION['is_admin']) && $_SESSION['is_admin'] == 1) {
    ...
    if (!empty($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
        $thumb_file_ext = strtolower(end(explode('.', $_FILES['image']['name'])));
    ...
    if ($uploadStatus == "success" || $uploadStatus == "") {
        if ($type_id == 1) {
            if (!empty($_FILES['article_image']['name']) && $_FILES['image']['name'] != '') {
                $article_file_ext = strtolower(end(explode('.', $_FILES['article_image']['name'])));
