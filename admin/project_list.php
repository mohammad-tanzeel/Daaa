    ...
    <!--            <script src="src/js/jquery-2.1.1.js" type="text/javascript"></script>-->
            ...
            <script type="text/javascript">
                ...
                function project_list() {
                    ...
                    $.getJSON(
                            'controller/get_project_list.php',
                            {
                                name: name,
                                type_id: type_id,
                                status: status,
                ...
                function  totalRecord() {
                    ...
                    var status = $('#status').val().trim();
                    $.ajax({
                        url: "controller/project_total.php",
                        type: 'POST',
                        data: {
                            name: name,
                            type_id: type_id,
                ...
                function addRow(row, sno) {
                    ...
                    $('table#project_list_table tr:last').append("<td>" + row.type_name + "</td>");
                    $('table#project_list_table tr:last').append("<td>" + row.location + "</td>");                                        
                    var status = row.p_status == 1 ? "Active" : "Inactive";
                    ...
                    var actionLink = "<td>\n\
                        <a href='add_project.php?id=" + row.id + "'  title='Edit Project'><i class='fa fa-pencil'></i></a> ";
                        actionLink += "<a href='#' onclick=view_gallery(" + row.id + ") title='View Project Gallery'>Gallery</a> ";
                    if (row.wnc_status == 0) {
                        actionLink += "<a href='#' onclick=delete_data(" + row.id + ") title='Delete Project'><i class='fa fa-trash-o'></i></a> ";
                ...
                {
                    window.location.href = "project_gallery.php?project_id="+id;
                }
